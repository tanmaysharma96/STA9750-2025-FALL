...
#| message: false
#| warning: false
#| fig-cap: "Health Care & Social Assistance vs. Total Employment (Animated, 2009–2023)"
...
# Use the dataset you built earlier:
# employment_health must have columns: GEOID, year, total_employment, health_employment
eh <- employment_health %>%
  dplyr::filter(is.finite(total_employment), is.finite(health_employment), is.finite(year))

if (nrow(eh) == 0L) {
  cat("Animation skipped (no rows in employment_health after filtering).")
} else {
  p_anim <- ggplot(eh, aes(x = total_employment, y = health_employment, group = GEOID)) +
    ...
    transition_time(year) +               # <-- uses lowercase 'year'
  ...
  anim <- animate(
    p_anim,
    nframes = length(unique(eh$year)) * 6,  # <-- use eh$year (not YEAR)
    fps = 10,
    width = 900,
    height = 600,
...
anim <- animate(
  p_anim,
  nframes = length(unique(eh$year)) * 6,   # <-- use eh$year, not employment_health$YEAR
...
rent_rank_clean <- rent_rank %>%
  mutate(
    NAME = enc2utf8(NAME),  # fix encoding for characters like é
...
datatable(
  ...
  options = list(
    ...
    autoWidth = TRUE,
    columnDefs = list(
      list(className = "dt-right", targets = c(1, 2))  # right-align numeric cols
    )
  ),
